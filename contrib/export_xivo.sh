...
sudo -u postgres psql --csv "${DB_NAME}" -c "select concat('vm-', voicemail.uniqueid) as ref, fullname as name, not cast(skipcheckpass as bool) as ask_password, attach as attach_audio, context, deletevoicemail as delete_messages, email, not cast(commented as bool) as enabled, language, maxmsg as max_messages, mailbox as number, options, pager, password, tz as timezone from voicemail" | ${DUMP} add --voicemails "${OUTPUT}"
